...
const state = {
  ...
  students     : new Map(),
  conductors   : new Set(),
  leaderboard  : new Map(),   // keyed by usn — latest result per player
};
...
function broadcastStudentList() {
  const list = Array.from(state.students.values()).map(s => ({
    ...
    terminated: s.terminated, online: s.online, result: s.result || null,
    currentRoom: s.currentRoom || null, fullscreen: s.fullscreen,
  }));
  io.to('conductors').emit('student_list_update', list);
}

function broadcastLeaderboard() {
  // Sort: score DESC, timeSec ASC
...
io.on('connection', (socket) => {
  ...
  socket.on('reset_leaderboard', () => {
    ...
    console.log('[CONDUCTOR] Leaderboard reset');
  });
  ...
  // ── Disconnect ────────────────────────────────────
  // ── Game result from student ─────────────────────
  socket.on('game_result', ({ name, usn, division, score, timeSec, log }) => {
    const resolvedUSN = (usn || '').trim().toUpperCase() || socket.id.slice(0,8);
    const entry = {
    ...
    // Only update if new result is better (higher score, or same score + faster time)
    const existing = state.leaderboard.get(resolvedUSN);
    if (!existing ||
        entry.score > existing.score ||
        (entry.score === existing.score && entry.timeSec < existing.timeSec)) {
      state.leaderboard.set(resolvedUSN, entry);
    }
